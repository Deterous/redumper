...
export int redumper(Options &options)
{
    ...
    if(aggregate.drive_required)
    {
        ...
        if(aggregate.drive_ready)
        {
            ...
            else if(!drive_is_recommended(ctx.drive_config.vendor_id, ctx.drive_config.product_id, ctx.drive_config.product_revision_level, ctx.drive_config.vendor_specific)
                && ctx.disc_type == DiscType::CD)
