...
export SPTD::Status read_sector(SPTD &sptd, uint8_t *sector, bool &unscrambled, const DriveConfig &drive_config, int32_t lba, bool flush_cache)
{
    ...
    {
        auto [omni_status, transferred] = cmd_read_omnidrive(sptd, sector_buffer.data(), CD_RAW_DATA_SIZE, lba, sectors_count, OmniDrive_DiscType::CD, false, flush_cache, false,
            OmniDrive_Subchannels::ENABLED, true);
        status = s;
        if(!status.status_code && transferred != sectors_count * CD_RAW_DATA_SIZE)
