...
export int redumper_debug(Context &ctx, Options &options)
{
    ...
    if(1)
    {
        ...
        LOG("0:");
        scrambler.descramble(sector0.data(), 0x030000, DATA_FRAME_SIZE, 0x8);
        ...
        LOG("1:");
        scrambler.descramble(sector1.data(), 0x030010, DATA_FRAME_SIZE, 0x8);
        LOG("  calc EDC: {:X}", DVD_EDC().update(sector1.data(), offsetof(DataFrame, edc)).final());
        write_vector("descrambled1.sector", sector1);
        LOG("11:");
        scrambler.descramble(sector11.data(), 0x030011, DATA_FRAME_SIZE, 0x8);
